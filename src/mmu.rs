...
impl Pde {
    #[must_use]
    pub const fn valid(self) -> bool { self.0 & (1 << 0) != 0 }
    #[must_use]
    pub const fn r#override(self) -> bool { self.0 & (1 << 1) != 0 }
    #[must_use]
    pub const fn read(self) -> bool { self.0 & (1 << 2) != 0 }
    #[must_use]
    pub const fn write(self) -> bool { self.0 & (1 << 3) != 0 }
    #[must_use]
    pub const fn execute(self) -> bool { self.0 & (1 << 4) != 0 }
    #[must_use]
    pub const fn next(self) -> u64 { self.0 & (!0b11_1111_1111_1111) }
    #[must_use]
    pub const fn has_perm(self, mode: AccessMode) -> bool { self.0 & (mode as u64) != 0 }
}
...
macro_rules! phys_get {
    ($fn_name: ident, $ty: ty) => {
        ...
        ///
        pub fn $fn_name(&self, addr: u64) -> Result<$ty, Response> { self.phys_get_sized(addr).map(|val| <$ty>::from_le_bytes(*val)) }
    };
}
macro_rules! phys_write {
    ($fn_name: ident, $ty: ty) => {
        ...
        /// Either [`Response::OutOfBounds`] or [`Response::Unaligned`] is returned as error. They should be straight forward.
        ///
        pub fn $fn_name(&mut self, addr: u64, with: $ty) -> Result<(), Response> { self.phys_write_sized(addr, &with.to_le_bytes()) }
    };
}
impl<T: AsRef<[u8]> + AsMut<[u8]>> Mmu<T> {
    ...
    pub fn size(&self) -> usize { self.memory.as_ref().len() }
    ...
    pub fn phys_get_sized<const SIZE: usize>(&self, addr: u64) -> Result<&[u8; SIZE], Response> {
        ...
        if addr >= self.size() {
            ...
            let Ok(res) = self.memory.as_ref()[addr..(addr + SIZE)].try_into() else { unreachable!() };
            Ok(res)
        }
    }
    /// # Errors
    ...
    /// Either [`Response::OutOfBounds`] or [`Response::Unaligned`] is returned as error. They should be straight forward.
    ///
    pub fn phys_write_sized<const SIZE: usize>(&mut self, addr: u64, with: &[u8; SIZE]) -> Result<(), Response> {
        let addr = usize::try_from(addr).map_err(|_| Response::OutOfBounds)?;
        if addr >= self.size() {
            Err(Response::OutOfBounds)
        } else if addr % SIZE != 0 {
    ...
    fn translate_address_level(&self, next: u64, level_index: u64, mode: AccessMode, auth_pde: &mut Option<Pde>) -> Result<u64, Response> {
