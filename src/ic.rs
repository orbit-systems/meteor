...
impl IntQueue {
    ...
    pub const fn new() -> Self {
        Self {
            data: [Interrupt(0); IntQueue::MAX_CAPACITY as usize],
            offset: 0,
            len: 0,
        }
    }
    #[must_use]
    pub const fn reached_capacity(&self) -> bool {
...
impl Index<usize> for IntQueue {
    ...
    fn index(&self, index: usize) -> &Self::Output {
        &self.data[self.loc(index)]
    }
}
impl IndexMut<usize> for IntQueue {
    fn index_mut(&mut self, index: usize) -> &mut Self::Output {
        &mut self.data[self.loc(index)]
    }
}
