...
pub trait Callback: Sized {
	...
	fn send_input<T: AsRef<[u8]> + AsMut<[u8]>>(
		&mut self,
		_state: &State<T>,
	) -> Option<(Port, u64)> {
		None
...
macro_rules! read {
	($fn_name: ident, $phys_get: ident, $ty: ty) => {
		...
		pub fn $fn_name(&self, addr: u64) -> Result<$ty, Response> {
			let addr = if self.cpu.get_flag(StFlag::MODE) == ProcMode::USER {
				self.mmu.translate_address(addr, AccessMode::Read)?
			} else {
				addr
			};
...
macro_rules! write {
	($fn_name: ident, $phys_write: ident, $ty: ty) => {
		...
		pub fn $fn_name(&mut self, addr: u64, value: $ty) -> Result<(), Response> {
			let addr = if self.cpu.get_flag(StFlag::MODE) == ProcMode::USER {
				self.mmu.translate_address(addr, AccessMode::Write)?
			} else {
				addr
			};
...
impl<T: AsRef<[u8]> + AsMut<[u8]>> State<T> {
	...
	pub const fn current_instr(&self) -> Instruction {
		self.cpu.registers.current_instr()
	}
	pub fn set_current_instr(&mut self, instr: Instruction) {
		self.cpu.registers.set_current_instr(instr);
	}
	...
	pub fn read_instruction(&self, addr: u64) -> Result<Instruction, Response> {
		let addr = if self.cpu.get_flag(StFlag::MODE) == ProcMode::USER {
			self.mmu.translate_address(addr, AccessMode::Execute)?
		} else {
			addr
		};
	...
	pub fn push_interrupt(&mut self, err: Interrupt) {
		...
		match self
			.mmu
			.phys_get_u64(self.ic.ivt_base_address + 8 * u64::from(err.0))
		{
			Err(err) => {
	...
	pub fn push_interrupt_from_mmu(&mut self, res: Response) {
		self.push_interrupt(match res {
			Response::AccViolation | Response::NoPerms | Response::OutOfBounds => {
				Interrupt::ACCESS_VIOLATION
			}
			Response::Unaligned => Interrupt::UNALIGNED_ACCESS,
		});
	}
	#[must_use]
	pub const fn regval(&self, reg: Register) -> u64 {
		self.cpu.registers.index(reg)
	}
	pub fn regval_mut(&mut self, reg: Register) -> &mut u64 {
		&mut self.cpu.registers[reg]
	}
	/// BE CAREFUL! opposite order than normal assignment.
	pub fn regval_write(&mut self, from: Register, to: Register) {
		*self.regval_mut(to) = self.regval(from);
	}
	...
	pub fn return_interrupt(&mut self) {
		...
		if self.ic.queue.is_empty() {
			...
			match self
				.mmu
				.phys_get_u64(self.ic.ivt_base_address + 8 * u64::from(code))
			{
	...
	pub fn push_stack_from(&mut self, reg: Register) {
		self.push_stack(self.regval(reg));
	}
	...
	pub fn interpret_code(&mut self) -> Result<(), Interrupt> {
		...
		let instr = self
			.current_instr()
			.try_into_instruction_set()
			.ok_or(Interrupt::INVALID_OPERATION)?;
		match instr {
			// system control
			I::Int { imm8 } => Err(imm8)?,
			I::Iret => {
			...
			I::Outr { rd, rs } => {
				self.proc_mode_is_user_then_invalid()?;
				self.ioc
					.send_out(Port(self.regval(rd) as u16), self.regval(rs));
			}
			I::Outi { imm16, rs } => {
				self.proc_mode_is_user_then_invalid()?;
			...
			I::Jal { rs, imm16 } => {
				...
				*self.regval_mut(Register::Ip) =
					self.regval(rs) + (sign_extend::<16>(imm16.into()) as i64 * 4) as u64;
			}
			I::Jalr { rd, rs, imm16 } => {
			...
			I::Branch { cc, imm20 } => {
				if self.cpu.cond(cc) {
					...
					*self.regval_mut(Register::Ip) = self
						.regval(Register::Ip)
						.wrapping_add((sign_extend::<20>(imm20.into()) as i64 * 4) as u64);
				}
			}
			// stack
			I::Push { rs } => {
			...
			I::Sw {
				...
				if let Err(err) =
					self.write_u64(self.load_address(rs, off, rn, sh), self.regval(rd))
				{
					self.push_interrupt_from_mmu(err);
				}
			}
			I::Sh {
				...
				if let Err(err) =
					self.write_u32(self.load_address(rs, off, rn, sh), self.regval(rd) as u32)
				{
					self.push_interrupt_from_mmu(err);
				}
			}
			I::Sq {
				...
				if let Err(err) =
					self.write_u16(self.load_address(rs, off, rn, sh), self.regval(rd) as u16)
				{
					self.push_interrupt_from_mmu(err);
				}
			}
			...
			I::Cmpi { r1, s, imm } => {
				if s {
					self.cpu
						.set_cmp_u64(sign_extend::<16>(imm.into()), self.regval(r1));
				} else {
					self.cpu
						.set_cmp_u64(self.regval(r1), sign_extend::<16>(imm.into()));
				}
			}
			...
			I::Fcmp { r1, r2, p } => {
				self.cpu
					.set_cmp_float_precision(self.regval(r1), self.regval(r2), p)
			}
			...
			I::Fadd { rd, r1, r2, p } => {
				*self.regval_mut(rd) = p.fadd(self.regval(r1), self.regval(r2))
			}
			I::Fsub { rd, r1, r2, p } => {
				*self.regval_mut(rd) = p.fsub(self.regval(r1), self.regval(r2))
			}
			I::Fmul { rd, r1, r2, p } => {
				*self.regval_mut(rd) = p.fmul(self.regval(r1), self.regval(r2))
			}
			I::Fdiv { rd, r1, r2, p } => {
				*self.regval_mut(rd) = p.fdiv(self.regval(r1), self.regval(r2))
			}
			I::Fma { rd, r1, r2, p } => {
				p.fma(self.regval(r1), self.regval(r2), self.regval_mut(rd))
			}
			I::Fsqrt { rd, r1, p } => *self.regval_mut(rd) = p.fsqrt(self.regval(r1)),
			I::Fmin { rd, r1, r2, p } => {
				*self.regval_mut(rd) = p.fmin(self.regval(r1), self.regval(r2))
			}
			I::Fmax { rd, r1, r2, p } => {
				*self.regval_mut(rd) = p.fmax(self.regval(r1), self.regval(r2))
			}
	...
	const fn carry(&self) -> bool {
		self.cpu.get_flag(StFlag::CARRY_BORROW_UNSIGNED)
	}
	/* arithmetic */
	fn arithmetic_r<Ret>(
		&mut self,
		func: impl Fn(&mut Self, u64, u64, Register) -> Ret,
		r1: Register,
		r2: Register,
		rd: Register,
	) -> Ret {
	...
	fn add(&mut self, a: u64, b: u64, to: Register) {
		let AddResult {
			...
			unsigned_overflow,
			signed_overflow,
		} = ops::add(a, b, self.carry());
		*self.regval_mut(to) = result;
		self.cpu
			.set_flag(StFlag::CARRY_BORROW_UNSIGNED, unsigned_overflow);
	...
	fn sub(&mut self, a: u64, b: u64, to: Register) {
		let AddResult {
			...
			unsigned_overflow,
			signed_overflow,
		} = ops::sub(a, b, self.carry());
		*self.regval_mut(to) = result;
		self.cpu
			.set_flag(StFlag::CARRY_BORROW_UNSIGNED, unsigned_overflow);
		self.cpu.set_flag(StFlag::CARRY_BORROW, signed_overflow);
	}
	fn imul(&mut self, a: u64, b: u64, to: Register) {
		*self.regval_mut(to) = ops::imul(a, b);
	}
	fn idiv(&mut self, a: u64, b: u64, to: Register) -> Result<(), Interrupt> {
		*self.regval_mut(to) = ops::idiv(a, b).ok_or(Interrupt::DIVIDE_BY_ZERO)?;
		Ok(())
	}
	fn umul(&mut self, a: u64, b: u64, to: Register) {
		*self.regval_mut(to) = ops::umul(a, b);
	}
	...
	fn rem(&mut self, a: u64, b: u64, to: Register) {
		*self.regval_mut(to) = ops::rem(a, b).unwrap_or(0);
	}
	fn r#mod(&mut self, a: u64, b: u64, to: Register) {
		*self.regval_mut(to) = ops::r#mod(a, b).unwrap_or(0);
	}
	/* bitwise */
	fn bitwise_r<Ret>(
		&mut self,
	...
	fn and(&mut self, a: u64, b: u64, to: Register) {
		*self.regval_mut(to) = ops::and(a, b);
	}
	fn or(&mut self, a: u64, b: u64, to: Register) {
		*self.regval_mut(to) = ops::or(a, b);
	}
	fn nor(&mut self, a: u64, b: u64, to: Register) {
		*self.regval_mut(to) = ops::nor(a, b);
	}
	fn xor(&mut self, a: u64, b: u64, to: Register) {
		*self.regval_mut(to) = ops::xor(a, b);
	}
	fn shl(&mut self, a: u64, b: u64, to: Register) {
		*self.regval_mut(to) = ops::shl(a, b);
	}
	fn asr(&mut self, a: u64, b: u64, to: Register) {
		*self.regval_mut(to) = ops::asr(a, b);
	}
	fn lsr(&mut self, a: u64, b: u64, to: Register) {
		*self.regval_mut(to) = ops::shr(a, b);
	}
	fn bit(&mut self, a: u64, b: u64, to: Register) {
		*self.regval_mut(to) = ops::bit(a, b);
	}
	...
	pub fn ioc_receive(&mut self, data: u64) {
		match self.ioc.ioc_data.status {
			...
			IocStatus::BindIntWaitingForInt => {
				self.ioc
					.bind_port(self.ioc.ioc_data.bindport, Interrupt(data as u8));
	...
	pub fn mmu_receive(&mut self, _data: u64) {
		todo!("mmu IO")
	}
	pub fn systimer_receive(&mut self, _data: u64) {
		todo!("systimer IO")
	}
...
impl State<std::boxed::Box<[u8]>> {
	...
	pub fn new_boxed(mem_cap: Option<std::num::NonZeroUsize>, image: &[u8]) -> Option<Self> {
		Some(Self {
			mmu: Mmu::new_boxed(mem_cap, image)?,
			ic: Ic::new(),
			cpu: Cpu::new(),
			ioc: Ioc::new(),
		})
...
impl<T: AsRef<[u8]> + AsMut<[u8]>, U: Callback> Emulator<T, U> {
	pub const fn new(state: State<T>, callback: U) -> Self {
		Self { state, callback }
	}
				...
				*self.state.regval_mut(Register::Ip) += 4;
				...
				if self.state.ioc.out_pin {
					...
					match self.state.ioc.port {
						...
						port => self.callback.on_output_received(
							&self.state,
							port,
							self.state.ioc.port_data(port),
						),
					}
