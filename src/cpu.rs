...
impl Registers {
	#[must_use]
	pub const fn index(self, index: Register) -> u64 {
	...
	pub const fn get_flag(&self, flag: StFlag) -> bool {
		(*self).index(Register::St) & flag.bits() != 0
	}
	...
	pub const fn current_instr(&self) -> Instruction {
		Instruction(((*self).index(Register::St) >> 32) as u32)
	}
	pub fn set_current_instr(&mut self, instr: Instruction) {
		self[Register::St].write::<1>(instr.0);
	}
...
impl Cpu {
	...
	pub const fn cond(&self, cond: BranchCond) -> bool {
		match cond {
			...
			BranchCond::Bleu => {
				self.get_flag(StFlag::LESS_UNSIGNED) || self.get_flag(StFlag::EQUAL)
			}
	...
	pub fn set_cmp(
		...
		self.registers
			.set_cmp(equal, less, less_unsigned, sign, zero);
	}
	#[allow(clippy::cast_possible_wrap)]
	pub fn set_cmp_u64(&mut self, a: u64, b: u64) {
		self.set_cmp(
			a == b,
			(a as i64) < (b as i64),
			a < b,
	...
	pub fn set_cmp_float<F: Float>(&mut self, a: F, b: F) {
		self.set_cmp(a == b, a < b, a < b, a.is_sign_negative(), a.is_zero());
	}
	pub fn set_cmp_float_bits<F: Float>(&mut self, a: u64, b: u64) {
		self.set_cmp_float(F::from_u64(a), F::from_u64(b));
	}
